...
async def crawl_generic_website(browser: Browser, start_url: str, county_name: str):
    """
    Crawls a website, intelligently routing page links to the browser and document links to a direct downloader.
    """
    ...
    logging.info(f"[{county_name}] Finished crawling pages. Found {len(doc_queue)} documents to download.")
    if doc_queue:
        tasks = [utils.download_and_process_file(doc_url, county_name) for doc_url in doc_queue]
